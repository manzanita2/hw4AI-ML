...
(`report/`) is a fillable LaTeX template (`design_justification.tex` +
`Makefile`) with the factual sections pre-filled and author-input boxes for
the rest.
...
| `report/design_justification.tex` | Design justification (LaTeX) | Fillable LaTeX template for the 9-section report. PSU-logo title page, factual sections pre-filled (synthesis/benchmark numbers, "what did not work"), inline `QUESTION FOR AUTHOR` boxes for the remaining inputs. ~2000-2500 words. |
...
| `report/figures/`             | Design justification (figures) | Report figures. `make_block_diagram.py` -> `block_diagram.png` (top.sv module graph), `make_dataflow_diagram.py` -> `dataflow_diagram.png` (weight-stationary dataflow + phase timeline). Drop the official PSU logo here as `psu_logo.png` for the title page. |
