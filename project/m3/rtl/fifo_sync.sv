...
// -------------------------------------------------------------------
// M3 targets sky130 via OpenLane 2 in elab-only / scope-attempt form.
// At the bring-up shape M = N = 4, the egress FIFO is 4 entries x 256
// bits = 1 kilobit and the ingress FIFO is 4 x 256 = 1 kilobit. Both
// are well under the area threshold where a real SRAM macro starts to
// pay off, and avoiding macro instantiation keeps the LibreLane /
// OpenLane flow self-contained (no OpenRAM step). For an M = N = 48
// override this would want a 1RW SRAM macro instead -- documented in
